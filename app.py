...
META = {
    "AFG GOV BOND FUND":       {"perf": 6.41,  "vol": 3.61,  "sharpe": 1.774, "sortino": 2.524, "alpha_j": -0.82, "beta": 0.79,  "te": 1.638, "ir": -1.291, "dd": -2.71,  "var99": -0.384},
    "AD BALANCED FUND":        {"perf": 33.23, "vol": 14.27, "sharpe": 2.210, "sortino": 3.242, "alpha_j": -1.99, "beta": 5.25,  "te": 12.287,"ir":  2.011, "dd": -17.80, "var99": -1.906},
    "AFG OPTIMAL FUND":        {"perf": 12.66, "vol": 9.68,  "sharpe": 1.179, "sortino": 1.422, "alpha_j": -8.68, "beta": 3.04,  "te": 7.320, "ir":  0.565, "dd": -7.75,  "var99": -1.573},
    "CDG IZDIHAR":             {"perf": 21.31, "vol": 10.42, "sharpe": 1.832, "sortino": 2.840, "alpha_j": -6.59, "beta": 4.09,  "te": 8.597, "ir":  1.488, "dd": -14.11, "var99": -1.293},
    "AD SELECT BANK":          {"perf": 3.50,  "vol": 0.55,  "sharpe": 2.305, "sortino": 3.063, "alpha_j":  0.69, "beta": 0.09,  "te": 2.056, "ir": -2.442, "dd": -0.19,  "var99": -0.081},
    "ALPHA BANQUES FUND":      {"perf": 3.23,  "vol": 0.48,  "sharpe": 2.024, "sortino": 2.706, "alpha_j":  0.46, "beta": 0.08,  "te": 2.056, "ir": -2.574, "dd": -0.18,  "var99": -0.078},
    "ALPHA SECURE FUND":       {"perf": 5.44,  "vol": 1.70,  "sharpe": 1.871, "sortino": 2.393, "alpha_j":  0.24, "beta": 0.47,  "te": 1.787, "ir": -1.728, "dd": -1.59,  "var99": -0.274},
    "CAM OBLIBANQUES":         {"perf": 3.32,  "vol": 0.63,  "sharpe": 1.965, "sortino": 2.741, "alpha_j":  0.53, "beta": 0.09,  "te": 2.063, "ir": -2.524, "dd": -0.17,  "var99": -0.074},
    "CDG RENDEMENT":           {"perf": 12.85, "vol": 2.36,  "sharpe": 4.600, "sortino": 6.264, "alpha_j":  5.81, "beta": 0.76,  "te": 1.670, "ir":  2.590, "dd": -3.68,  "var99": -0.283},
    "OBLIG CT":                {"perf": 3.40,  "vol": 0.54,  "sharpe": 2.141, "sortino": 2.652, "alpha_j":  0.61, "beta": 0.09,  "te": 2.063, "ir": -2.483, "dd": -0.19,  "var99": -0.096},
    "CDG TAWFIR":              {"perf": 5.58,  "vol": 1.91,  "sharpe": 1.762, "sortino": 2.278, "alpha_j": -0.52, "beta": 0.61,  "te": 1.576, "ir": -1.868, "dd": -1.69,  "var99": -0.344},
    "EMERGENCE SERENITE":      {"perf": 10.73, "vol": 2.27,  "sharpe": 3.739, "sortino": 5.365, "alpha_j":  3.17, "beta": 0.85,  "te": 1.353, "ir":  1.630, "dd": -3.60,  "var99": -0.616},
    "CAPITAL TRUST EQUILIBRE": {"perf": 11.92, "vol": 9.11,  "sharpe": 1.178, "sortino": 1.420, "alpha_j": -8.53, "beta": 2.90,  "te": 6.662, "ir":  0.510, "dd": -7.63,  "var99": -1.710},
    "AD YIELD FUND":           {"perf": 3.22,  "vol": 0.58,  "sharpe": 1.681, "sortino": 2.277, "alpha_j":  0.43, "beta": 0.09,  "te": 2.073, "ir": -2.561, "dd": -0.21,  "var99": -0.089},
}
...
def calcul_stats(w_arr, R=R_GLOBAL):
    ...
    sharpe  = (perf/100 - RF) / (vol/100) if vol > 0 else 0
    r_neg   = r_ptf[r_ptf < RF_DAILY]
    dv      = r_neg.std() * np.sqrt(252) if len(r_neg) > 1 else vol/100
    sortino = (perf/100 - RF) / dv if dv > 0 else 0
    var99   = np.percentile(r_ptf, 1, method="lower") * 100
    cvar99  = r_ptf[r_ptf < var99/100].mean() * 100 if (r_ptf < var99/100).any() else var99
...
def optimiser(methode, w_actuel, R=R_GLOBAL):
    ...
    def cvar_fn(w):
        ...
        return -r[r <= v].mean() if (r <= v).any() else -v
    ...
    objectives = {
        ...
        "Score Composite": lambda w: -(
            np.array([META[n]["sharpe"]  for n in NOMS]) / 4.6 * 0.25 +
            np.array([max(META[n]["alpha_j"], 0) for n in NOMS]) / 5.81 * 0.35 +
            np.array([META[n]["perf"] / 100 for n in NOMS]) / 0.33 * 0.25 +
            np.array([max(-META[n]["dd"], 0) / 17.8 for n in NOMS]) * 0.15
        ) @ w + cvar_fn(w) * 2,
    }
    ...
    for _ in range(5):
        ...
        w0 = np.clip(w0, 0.01, 0.25); w0 /= w0.sum()
        starts.append(w0)
    ...
    for w0 in starts:
        try:
            ...
            pass
    return best.x if best else w_actuel


@st.cache_data
def calculer_frontiere(R=R_GLOBAL):
    """Calcule la frontière efficiente (120 points, cachée)."""
    mu  = R.mean(axis=0) * 252
    cov = np.cov(R.T) * 252
    n   = R.shape[1]
    ...
    ret_min = mu.min()
    ret_max = mu.max() * 0.82
    targets = np.linspace(ret_min, ret_max, 120)
    ...
    for t in targets:
        ...
        if best and best.success:
            ...
            var99s.append(np.percentile(r_ptf, 1, method="lower") * 100)
...
CATEGORIES = {
    "🏦 Oblig. MLT": ["AFG GOV BOND FUND","ALPHA SECURE FUND","CAM OBLIBANQUES","CDG RENDEMENT","OBLIG CT","CDG TAWFIR","EMERGENCE SERENITE"],
    "📈 Diversifié":  ["AD BALANCED FUND","AFG OPTIMAL FUND","CDG IZDIHAR","CAPITAL TRUST EQUILIBRE"],
    "💰 Oblig. CT":   ["AD SELECT BANK","ALPHA BANQUES FUND","AD YIELD FUND"],
}
    ...
    with st.sidebar.expander(cat, expanded=True):
        for nom in fonds:
            poids_user[nom] = st.slider(
                nom.replace("FCP ", "").replace("SICAV ", "")[:22],
                min_value=0.0, max_value=25.0,
                value=float(POIDS_ACTUELS[nom]),
...
with col_sb1:
    if st.button("🔄 Réinitialiser", use_container_width=True):
        ...
        st.rerun()
with col_sb2:
    methode_optim = st.selectbox("", ["Min CVaR","Min Variance","Max Sharpe","Score Composite"], label_visibility="collapsed")
...
if st.sidebar.button(f"⚡ Appliquer {methode_optim}", use_container_width=True, type="primary"):
    ...
    with st.spinner(f"Optimisation {methode_optim}..."):
...
st.sidebar.markdown(f"""
<div style="margin-top:12px;padding:8px;background:{COLORS['dark_green']};border-radius:6px;font-size:0.75rem;color:{COLORS['gray']}">
...
with tab1:
    ...
    kpis = [
        ("Perf. Annuelle",     f"{stats_cur['perf']:.2f}%",  stats_cur["perf"],  stats_ref["perf"],  "%",  False, ""),
        ("Volatilité/an",      f"{stats_cur['vol']:.2f}%",   stats_cur["vol"],   stats_ref["vol"],   "%",  True,  "red"),
    ...
    with col_left:
        ...
        fig_pie = go.Figure(go.Pie(
            labels=[n.replace("FCP ","").replace("SICAV ","")[:18] for n in NOMS],
            values=poids_pct,
    ...
    with col_right:
        st.markdown('<div class="section-title">📊 Poids Courant vs Poids Actuel</div>', unsafe_allow_html=True)

        noms_short = [n.replace("FCP ","").replace("SICAV ","")[:18] for n in NOMS]
        ...
        fig_bar.add_hline(y=1,  line_dash="dot", line_color=COLORS["light_green"], line_width=1, annotation_text="min 1%")
        fig_bar.add_hline(y=25, line_dash="dot", line_color=COLORS["red"],         line_width=1, annotation_text="max 25%")
    ...
    fig_cum.add_trace(go.Scatter(
        ...
        line=dict(color=COLORS["gray"], dash="dash", width=1.8), opacity=0.7
    ))
    fig_cum.add_trace(go.Scatter(
        x=t_idx, y=cum_cur * 100, name="Portefeuille Courant",
        line=dict(color=COLORS["mid_green"], width=2.5),
        fill="tonexty" if cum_cur[-1] > cum_ref[-1] else None,
        fillcolor="rgba(44,95,45,0.08)"
    ))
...
with tab2:
    ...
    for nom in NOMS:
        ...
        fig_fe.add_trace(go.Scatter(
            x=[m["vol"]], y=[m["perf"]],
            mode="markers+text",
            marker=dict(size=8, color=COLORS["light_green"] if aj > 0 else COLORS["red"],
                        symbol="circle", opacity=0.7,
                        line=dict(color="white", width=0.8)),
            text=[nom.replace("FCP ","").replace("SICAV ","")[:12]],
            textposition="top right", textfont=dict(size=7, color=COLORS["gray"]),
            name=nom[:20], showlegend=False,
            hovertemplate=f"<b>{nom}</b><br>Vol: {m['vol']:.2f}%<br>Perf: {m['perf']:.2f}%<br>Sharpe: {m['sharpe']:.2f}<br>α Jensen: {aj:+.2f}%<extra></extra>",
        ))
    ...
    fig_fe.add_trace(go.Scatter(
        ...
        mode="markers+text",
        marker=dict(size=16, color=COLORS["gray"], symbol="square",
    ...
    fig_fe.add_trace(go.Scatter(
        ...
        mode="markers+text",
        marker=dict(size=20, color=COLORS["light_green"], symbol="star",
                    line=dict(color=COLORS["dark_green"], width=1.5)),
    ...
    for m_name, s in optim_points.items():
        fig_fe.add_trace(go.Scatter(
            x=[s["vol"]], y=[s["perf"]],
            mode="markers+text",
            marker=dict(size=14, color=col_opt[m_name], symbol=markers_m[m_name],
                        line=dict(color="white", width=1.2)),
            text=[m_name[:10]], textposition="bottom right",
            textfont=dict(size=8, color=col_opt[m_name]),
            name=m_name,
    ...
    # Ligne du marché (CML)
    if len(fe_vols) > 0:
        best_sh_idx = np.argmax([(fe_rets[i]/100 - RF) / (fe_vols[i]/100) for i in range(len(fe_vols))])
        slope_cml   = (fe_rets[best_sh_idx]/100 - RF) / (fe_vols[best_sh_idx]/100)
        ...
        fig_fe.add_trace(go.Scatter(
            ...
            line=dict(color=COLORS["gold"], dash="dot", width=1.5), opacity=0.7,
        ))
        fig_fe.add_annotation(
            x=0, y=RF * 100, text=f"Rf={RF*100:.2f}%",
            showarrow=False, font=dict(size=9, color=COLORS["gold"]),
            xanchor="left"
        )
    ...
    fig_fe.update_layout(
        ...
        yaxis=dict(title="Rendement annualisé (%)",   gridcolor="#E2E8F0", zeroline=False),
    ...
    st.markdown('<div class="section-title">📊 Résumé des 4 Méthodes</div>', unsafe_allow_html=True)
    rows_m = []
    for m_name, s in optim_points.items():
        rows_m.append({
            "Méthode": m_name,
    ...
    st.dataframe(df_m.set_index("Méthode"), use_container_width=True)
...
with tab3:
    ...
    for i, nom in enumerate(NOMS):
        ...
        fig_sc.add_trace(go.Scatter(
            ...
            mode="markers+text",
            marker=dict(size=max(wp * 2.5, 6), color=col, opacity=0.7,
    ...
    fig_sc.add_vline(x=0, line_dash="dot", line_color=COLORS["red"],    annotation_text="α=0")
    fig_sc.add_hline(y=2, line_dash="dot", line_color=COLORS["navy"],   annotation_text="Sharpe=2")
...
# ═══════════════════════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="section-title">⚖️ Comparaison Visuelle des 4 Méthodes</div>', unsafe_allow_html=True)
    ...
    with col_r:
        ...
        for i in range(N_cat):
            vals = {k: radar_vals[k][i] for k in all_ptfs}
            mn, mx = min(vals.values()), max(vals.values())
            for k in all_ptfs:
                radar_norm[k].append((vals[k] - mn) / (mx - mn + 1e-10))

        fig_radar = go.Figure()
        for nm in all_ptfs:
            v = radar_norm[nm] + [radar_norm[nm][0]]
            a = angles
            fig_radar.add_trace(go.Scatterpolar(
                r=v, theta=cats + [cats[0]],
                fill="toself", name=nm,
                line=dict(color=col_radar[nm], width=2),
                fillcolor=col_radar[nm], opacity=0.12,
            ))
        fig_radar.update_layout(
            polar=dict(radialaxis=dict(visible=False, range=[0, 1])),
            showlegend=True, height=380,
            legend=dict(font=dict(size=9), x=0.8, y=1.1),
            paper_bgcolor="rgba(0,0,0,0)",
    ...
    with col_h:
        # Heatmap des poids
        noms_short = [n.replace("FCP ","").replace("SICAV ","")[:16] for n in NOMS]
        ...
        fig_hm.update_layout(
            ...
            margin=dict(t=20, b=80, l=100, r=40),
        )
        st.plotly_chart(fig_hm, use_container_width=True)

    # VaR bar chart comparatif
    st.markdown('<div class="section-title">📉 Réduction VaR 99% par Méthode</div>', unsafe_allow_html=True)
    ...
    cols_bar2 = [COLORS["gray"], COLORS["mid_green"], COLORS["navy"], COLORS["red"], "#6B3FA0"]

    fig_var.add_trace(go.Bar(
        name="VaR 99%", x=noms_ptf, y=vars_ptf,
        marker_color=cols_bar2, opacity=0.85,
        text=[f"{v:.4f}%" for v in vars_ptf], textposition="inside",
        textfont=dict(color="white", size=10),
    ))
    fig_var.add_trace(go.Bar(
        name="CVaR", x=noms_ptf, y=cvars_ptf,
        marker_color=cols_bar2, opacity=0.45, hatch="//",
        text=[f"{v:.4f}%" for v in cvars_ptf], textposition="inside",
    ...
    # Sharpe comparison
    st.markdown('<div class="section-title">⭐ Sharpe & Sortino par Méthode</div>', unsafe_allow_html=True)
    fig_sh = go.Figure()

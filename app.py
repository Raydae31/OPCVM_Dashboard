...
Valeurs META corrigées avec les vraies données historiques
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# DONNÉES — VALEURS RÉELLES ISSUES DU TABLEAU HISTORIQUE
# ══════════════════════════════════════════════════════════════════════════════
...
META = {
    # Toutes les valeurs sont issues du tableau réel des indicateurs par OPCVM
    # perf=Performance%, vol=Volatilité%, alpha_j=Alpha Jensen%, beta=Beta,
    # sharpe=Ratio Sharpe, sortino=Ratio Sortino, dd=Drawdown Max%,
    # var99=VaR 99%/jour, cvar99=CVaR 99%/jour, te=Tracking Error%, ir=Info Ratio
    ...
    "CDG RENDEMENT":           {"perf":  5.06, "vol":  5.05, "sharpe": 4.58, "sortino": 6.49, "alpha_j": -0.28, "beta": 0.66, "te":  3.48, "ir": -0.38, "dd": -1.69, "var99": -0.633,  "cvar99": -0.715},
    ...
    "EMERGENCE SERENITE":      {"perf":  4.63, "vol":  4.83, "sharpe": 4.31, "sortino": 5.79, "alpha_j":  0.06, "beta": 1.10, "te":  2.34, "ir":  0.12, "dd": -1.66, "var99": -0.616,  "cvar99": -0.696},
...
def optimiser(methode, w_actuel, R=R_GLOBAL):
    ...
    # VaR & CVaR pondérées (vraies valeurs)
    var99_ind  = np.array([META[n]["var99"]  for n in NOMS])
    cvar99_ind = np.array([META[n]["cvar99"] for n in NOMS])

    def cvar_fn(w):
        return float(w @ cvar99_ind)   # minimiser CVaR pondérée (valeurs négatives → minimiser = chercher le moins négatif)
        ...
        for nom in NOMS:
            ...
            fig_fe.add_trace(go.Scatter(
                ...
                hovertemplate=(f"<b>{nom}</b><br>Vol: {m['vol']:.2f}%<br>Perf: {m['perf']:.2f}%<br>"
                               f"Sharpe: {m['sharpe']:.3f}<br>α: {m['alpha_j']:+.2f}%<extra>OPCVM</extra>"),
            ))
...
<div style="text-align:center;padding:20px;">
  <p style="color:{COLORS['gray']};font-size:0.75rem;margin:0;">
    <b>OPCVM Portfolio Dashboard v3</b> — Valeurs META réelles (tableau indicateurs OPCVM).<br>
    VaR/CVaR = agrégation pondérée des valeurs historiques individuelles réelles.<br>
    Rf = 2.25% · 247 jours · 3 méthodes : Min Variance · Max Sharpe · Min CVaR
  </p>
